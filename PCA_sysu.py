...
def main():
    ...
    main_worker_stage1(args,log_s1_name,subdir)
    # subdir='20230806'

    main_worker_stage2(args,log_s1_name,log_s2_name,subdir) #add CMA

def main_worker_stage1(args,log_s1_name,subdir):
    start_epoch=0
    best_mAP=0
...
def main_worker_stage2(args,log_s1_name,log_s2_name,subdir,update_subdir=None):
    start_epoch=0
    best_mAP=0
    # logs_dir_stage1=args.logs_dir
    ...
    for epoch in range(args.epochs):
        with torch.no_grad():
            ...
            rerank_dist_all = compute_modal_invariant_jaccard_distance(feature_all, k1=40, k2=32,file=sorted(dataset_rgb.train)+sorted(dataset_ir.train),
                                           search_option=3)  # rerank_dist_all_jacard[:features_rgb.size(0),:features_rgb.size(0)]# ##40 32
            pseudo_labels_all = cluster_all.fit_predict(rerank_dist_all)
